...
Local development needs no build — just open `index.html` and the browser
transpiles the inline JSX on the fly (using `vendor/babel.min.js`).

For deployment, `node build.mjs` pre-compiles that JSX once and writes a
production copy to `dist/` that ships only the React runtime (no ~3 MB Babel,
no in-browser transpile). The GitHub Actions workflow in
`.github/workflows/deploy-pages.yml` runs this build and publishes `dist/` to
GitHub Pages on every push to `master`. `dist/` is gitignored — it's a build
artifact, regenerated by CI.
